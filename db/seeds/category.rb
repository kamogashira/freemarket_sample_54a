...
#親カテゴリー毎ブロックに分け、子と孫カテゴリーの作成

#レディースブロック

#レディースの子カテゴリー配列
ladys_children = ['トップス','ジャケット/アウター','パンツ','スカート','ワンピース','靴','ルームウェア/パジャマ','レッグウェア','帽子','バッグ','アクセサリー','ヘアアクセサリー','小物','時計','ウィッグ/エクステ','浴衣/水着','スーツ/フォーマル/ドレス','マタニティ','その他']
#レディースの孫カテゴリー配列
ladys_grandchildren = [['Tシャツ/カットソー(半袖/袖なし)','Tシャツ/カットソー(七分/長袖)','シャツ/ブラウス(半袖/袖なし)','シャツ/ブラウス(七分/長袖)','ポロシャツ','キャミソール','タンクトップ','ホルターネック','ニット/セーター','チュニック','カーディガン/ボレロ','アンサンブル','ベスト/ジレ','パーカー','トレーナー/スウェット','ベアトップ/チューブトップ','ジャージ','その他'],
...
ladys_children.each_with_index do |child, i|
  child = Category.find_by(name: 'レディース').children.create(name: child)
  ladys_grandchildren[i].each do |grandchild|
...
#メンズの子カテゴリー配列
mens_children = ['トップス','ジャケット/アウター','パンツ','靴','バッグ','スーツ','帽子','アクセサリー','小物','時計','水着','レッグウェア','アンダーウェア','その他']
#メンズの孫カテゴリー配列
mens_grandchildren = [['Tシャツ/カットソー(半袖/袖なし)','Tシャツ/カットソー(七分/長袖)','シャツ','ポロシャツ','タンクトップ','ニット/セーター','パーカー','カーディガン','スウェット','ジャージ','ベスト','その他'],
...
mens_children.each_with_index do |child, i|
  child = Category.find_by(name: 'メンズ').children.create(name: child)
  mens_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
#ベビー・キッズの子カテゴリー配列
babys_kids_children = ['ベビー服(女の子用) ~95cm','ベビー服(男の子用) ~95cm','ベビー服(男女兼用) ~95cm','キッズ服(女の子用) 100cm~','キッズ服(男の子用) 100cm~','キッズ服(男女兼用) 100cm~','キッズ靴','子ども用ファッション小物','おむつ/トイレ/バス','外出/移動用品','授乳/食事','ベビー家具/寝具/室内用品','おもちゃ','行事/記念品','その他']
#ベビー・キッズの孫カテゴリー配列
babys_kids_grandchildren = [['トップス','アウター','パンツ','スカート','ワンピース','ベビードレス','おくるみ','下着/肌着','パジャマ','ロンパース','その他'],
...
babys_kids_children.each_with_index do |child, i|
  child = Category.find_by(name: 'ベビー・キッズ').children.create(name: child)
  babys_kids_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
home_children = ['キッチン/食器','ベッド/マットレス','ソファ/ソファベッド','椅子/チェア','机/テーブル','収納家具','ラグ/カーペット/マット','カーテン/ブラインド','ライト/照明','寝具','インテリア小物','季節/年中行事','その他']
#インテリア・住まい・小物の孫カテゴリー配列
home_grandchildren = [['食器','調理器具','収納/キッチン雑貨','弁当用品','カトラリー(スプーン等)','テーブル用品','容器','エプロン','アルコールグッズ','浄水機','その他'],
...
home_children.each_with_index do |child, i|
  child = Category.find_by(name: 'インテリア・住まい・小物').children.create(name: child)
  home_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
#本・音楽・ゲームの子カテゴリー配列
books_music_game_children = ['本','漫画','雑誌','CD','DVD/ブルーレイ','レコード','テレビゲーム']
#本・音楽・ゲームの孫カテゴリー配列
books_music_game_grandchildren = [['文学/小説','人文/社会','ノンフィクション/教養','地図/旅行ガイド','ビジネス/経済','健康/医学','コンピュータ/IT','趣味/スポーツ/実用','住まい/暮らし/子育て','アート/エンタメ','洋書','絵本','参考書','その他'],
...
books_music_game_children.each_with_index do |child, i|
  child = Category.find_by(name:'本・音楽・ゲーム').children.create(name: child)
  books_music_game_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
toys_hobby_children = ['おもちゃ','タレントグッズ','コミック/アニメグッズ','トレーディングカード','フィギュア','楽器/器材','コレクション','ミリタリー','美術品','アート用品','その他']
#おもちゃ・ホビー・グッズの孫カテゴリー配列
toys_hobby_grandchildren = [['キャラクターグッズ','ぬいぐるみ','小物/アクセサリー','模型/プラモデル','ミニカー','トイラジコン','プラモデル','ホビーラジコン','鉄道模型','その他'],
...
toys_hobby_children.each_with_index do |child, i|
  child = Category.find_by(name:'おもちゃ・ホビー・グッズ').children.create(name: child)
  toys_hobby_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
#コスメ・香水・美容の子カテゴリー配列
cosme_children = ['ベースメイク','メイクアップ','ネイルケア','香水','スキンケア/基礎化粧品','ヘアケア','ボディケア','オーラルケア','リラクゼーション','ダイエット','その他']
#コスメ・香水・美容の孫カテゴリー配列
cosme_grandchildren = [['ファンデーション','化粧下地','コントロールカラー','BBクリーム','CCクリーム','コンシーラー','フェイスパウダー','トライアルセット/サンプル','その他'],
...
cosme_children.each_with_index do |child, i|
  child = Category.find_by(name:'コスメ・香水・美容').children.create(name: child)
  cosme_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
#家電・スマホ・カメラの子カテゴリー配列
appliance_phone_camera_children = ['スマートフォン/携帯電話','スマホアクセサリー','PC/タブレット','カメラ','テレビ/映像機器','オーディオ機器','美容/健康','冷暖房/空調','生活家電','その他']
#家電・スマホ・カメラの孫カテゴリー配列
appliance_phone_camera_grandchildren = [['スマートフォン本体','バッテリー/充電器','携帯電話本体','PHS本体','その他'],
...
appliance_phone_camera_children.each_with_index do |child, i|
  child = Category.find_by(name:'家電・スマホ・カメラ').children.create(name: child)
  appliance_phone_camera_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
#スポーツ・レジャーの子カテゴリー配列
sports_children = ['ゴルフ','フィッシング','自転車','トレーニング/エクササイズ','野球','サッカー/フットサル','テニス','スノーボード','スキー','その他スポーツ','アウトドア','その他']
#スポーツ・レジャーの孫カテゴリー配列
sports_grandchildren = [['クラブ','ウエア(男性用)','ウエア(女性用)','バッグ','シューズ(男性用)','シューズ(女性用)','アクセサリー','その他'],
...
sports_children.each_with_index do |child, i|
  child = Category.find_by(name:'スポーツ・レジャー').children.create(name: child)
  sports_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
#ハンドメイドの子カテゴリー配列
handmade_children = ['アクセサリー(女性用)','ファッション/小物','アクセサリー/時計','日用品/インテリア','趣味/おもちゃ','キッズ/ベビー','素材/材料','二次創作物','その他']
#ハンドメイドの孫カテゴリー配列
handmade_grandchildren = [['ピアス','イヤリング','ネックレス','ブレスレット','リング','チャーム','ヘアゴム','アンクレット','その他'],
...
handmade_children.each_with_index do |child, i|
  child = Category.find_by(name:'ハンドメイド').children.create(name: child)
  handmade_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
#チケットの子カテゴリー配列
tiket_children = ['音楽','スポーツ','演劇/芸能','イベント','映画','施設利用券','優待券/割引券','その他']
#チケットの孫カテゴリー配列
tiket_grandchildren = [['男性アイドル','女性アイドル','韓流','国内アーティスト','海外アーティスト','音楽フェス','声優/アニメ','その他'],
...
tiket_children.each_with_index do |child, i|
  child = Category.find_by(name:'チケット').children.create(name: child)
  tiket_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
#自動車・オートバイの子カテゴリー配列
car_bike_children = ['自動車本体','自動車タイヤ/ホイール','自動車パーツ','自動車アクセサリー','オートバイ車体','オートバイパーツ','オートバイアクセサリー']
#自動車・オートバイの孫カテゴリー配列
car_bike_grandchildren = [['国内自動車本体','外国自動車本体'],
...
car_bike_children.each_with_index do |child, i|
  child = Category.find_by(name:'自動車・オートバイ').children.create(name: child)
  car_bike_grandchildren[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
#その他の子カテゴリー配列
others_children = ['まとめ売り','ペット用品','食品','飲料/酒','日用品/生活雑貨/旅行','アンティーク/コレクション','文房具/事務用品','事務/店舗用品','その他']
#その他の孫カテゴリー配列
...
others_children.each_with_index do |child, i|
  child = Category.find_by(name:'その他').children.create(name: child)
